...
def main(args):
    ...
    resize = args.resize
    random_cam_nums = args.random_cam_nums
    ...
    for action_key in action_dict:
        ...
        for num_key in num_dict:
            ...
            for case_key in tqdm(case_dict):
                ...
                tmp_target_nums = min(len(cam_dict), random_cam_nums)
                target_cam_nums = np.random.choice(list(cam_dict.keys()), tmp_target_nums, replace=False)
                cam_dict = {x: cam_dict[x] for x in target_cam_nums}
...
def parse_args():
    ...
    out_dir = "/media/daton/Data/datasets/aihub/human_behavior_images2"
    ...
    parser.add_argument("--random-cam-nums", type=int, default=2)
